...
class ReplayMemory(object):
    ...
    def push(self, *args):
        """Saves a transition."""
        if len(self.self.memory) < self.capacity:
            self.self.memory.append(None)
        self.self.memory[self.position] = Transition(*args)
        self.position = (self.position + 1) % self.capacity

    def sample(self, batch_size):
        return random.sample(self.self.memory, batch_size)
